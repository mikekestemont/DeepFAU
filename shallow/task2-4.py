...
def main():
    ...
    images, filenames = [], []
    for fn in glob.glob('../data/ICDAR_CLAMM_task_2_4/*'):
        if fn.endswith('.db'):
            continue
        print(fn)
    ...
    with open('../output/task2_clean_script_belonging.txt',
              'w') as f:
    ...
    with open('../output/task4_clean_date_belonging.txt',
              'w') as f:
    ...
    with open('../output/task2_clean_script_distance.txt',
              'w') as f:
        f.write('L/C ,')
    ...
    dm = squareform(pdist(X, minmax))
    dm = dm / dm.sum(axis=1)[:, np.newaxis]
    with open('../output/task4_clean_date_distance.txt',
              'w') as f:
        f.write('L/C ,')
        f.write(', '.join(filenames)+'\n')
        for n, v in zip(filenames, dm):
